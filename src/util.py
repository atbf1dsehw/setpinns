...
def get_data(x_range, y_range, x_num, y_num):
    ...
    data = np.concatenate(
        (np.expand_dims(x_mesh, -1), np.expand_dims(t_mesh, -1)), axis=-1
    )
...
def make_set_sequence(src, num_step=5):
    return src.reshape(-1, num_step, 1)


def get_set_data(x_range, y_range, x_num, y_num):
    x = np.linspace(x_range[0], x_range[1], x_num)
    t = np.linspace(y_range[0], y_range[1], y_num)
    t_mesh, x_mesh = np.meshgrid(t, x)
    data = np.concatenate(
        (np.expand_dims(t_mesh, -1), np.expand_dims(x_mesh, -1)), axis=-1
    )
    b_lower = data[:, 0, :]
    b_upper = data[:, -1, :]
    b_left = data[0, :, :]
    b_right = data[-1, :, :]
    res = data.reshape(x_num * y_num, -1)
    return res, b_left, b_right, b_upper, b_lower
...
def get_data_3d(x_range, y_range, t_range, x_num, y_num, t_num):
    ...
    data = np.concatenate(
        (
            np.expand_dims(x_mesh, -1),
            np.expand_dims(y_mesh, -1),
            np.expand_dims(t_mesh, -1),
        ),
        axis=-1,
    )
